...
var ExperimentalItemView = Backbone.Marionette.ItemView.extend({
  template: "#experimental-template",
  className: "row-fluid",
});

var ExperimentalView = Backbone.Marionette.CollectionView.extend({
...
var AllJobView = Backbone.Marionette.ItemView.extend({
  template: "#alljobs-template",
});

var AllJobsView = Backbone.Marionette.CollectionView.extend({
  itemView: AllJobView
});

var AllJobDescriptionView = Backbone.Marionette.ItemView.extend({
  template: "#alljobs-description-template",
});

var AllJobsDescriptionView = Backbone.Marionette.CollectionView.extend({
  itemView: AllJobDescriptionView
});

var PageJobView = Backbone.Marionette.ItemView.extend({
  template: "#pagejob-template",
});

var PageJobsView = Backbone.Marionette.CompositeView.extend({
...
var MenuHeaderView = Backbone.Marionette.CompositeView.extend({
  template: "#menuheader-template",
});

var DocumentationView = Backbone.Marionette.ItemView.extend({
  template: "#documentation-template",
});

var NoDataView = Backbone.Marionette.ItemView.extend({
  template: "#nodata-template",
});

var SubTextView = Backbone.Marionette.ItemView.extend({
...
function tagDescriptionFilter(collection,tag) {
  ...
  _.each(myjobs, function(job) {
      qls.push(new Job({uuid: job.uuid, title: job.title, url: job.url, 
                        description: job.description, city: job.options.city, 
